...
Это общий вид СЛАУ, причём $a_{ij}, x_k, b_r\in K$

\textbf{Матричная форма записи} \(Ax = b\), где

\(\underbracket{A}_{\text{матрица системы}} = (a_{ij})_{m\times n} =
\begin{pmatrix}
    a_{11} & \ldots & a_{1n} \\
    \vdots & \ddots & \vdots \\
    a_{m1} & \ldots & a_{mn}
\end{pmatrix}\),
\(x = \underbracket{\begin{pmatrix}
    x_1 \\
    x_2 \\ \vdots \\ x_n \\
\end{pmatrix}}_{\text{столбец неизвестных}}\),
\(b = \underbracket{\begin{pmatrix}
    b_1 \\
    b_2 \\ \vdots \\ b_m \\
\end{pmatrix}}_{\text{столбец свободных членов}}\)

\(A = (A_1, \ldots, A_n)\), где $A_j$ --- столбцы. \fbox{\(\displaystyle \sum_{i=1}^{n} x_iA_i = b\)}  --- \textbf{векторная форма записи}
...
Система \(Ax = b\) --- \textbf{совместная (разрешимая)}, если \(\exists x\), то есть существует решение.

Система \(Ax = b\) --- \textbf{несовместная (неразрешимая)}, если \(\not\exists x\), то есть решения не существует.
...
\begin{itemize}
    ...
    \item \(1\leq k < n\):
          ...
          Тогда \(\rg(A) = k = \rg_{col}(A) = \rg(A_1, \ldots, A_n)\) --- база столбцов из \(k\) элементов. Переставим неизвестные $x_i$, чтобы базисом были столбцы \(A_1,\ldots, A_k\), а все остальные столбцы будут их линейными комбинациями.

          $\forall j = 1, \ldots, n-k:$
          
          \(A_{k + j} = \sum\limits_{i = 1}^k \alpha_i^j A_i\), где \(\alpha_i^j \in K\). (\(j\) --- тоже индекс, просто для удобства записанный сверху)
          ...
          \(u_1, \ldots, u_{n - k}\) --- решения \(Ax = \mathbb{0}\), причём линейно независимые, т.к. ни один не является линейной комбинацией остальных.
...
\begin{itemize}
    ...
    \item \(\rg(A) = n \Rightarrow\) система определенная, имеет единственное решение равное нулю, то есть система тривиальная.
\end{itemize}
...
\end{itemize}
\textbf{Следствия (если система $Ax=0$ совместна, то)}:
\begin{enumerate}
...
\prooff{}
          \(\forall b \in K^m Ax = b\) совместна \(\Leftrightarrow b = \sum\limits_{i=1}^n x_iA_i  \Leftrightarrow b \in span(A_1,\ldots,A_n)\)
          ...
          Заметим, что \(K^m \subset span(A_1,\ldots A_n) \subset K^m\), потому что любой базисный вектор  содержится в нашей оболочке. Откуда:

          \(span(A_1,\ldots A_n)  = K^m \Rightarrow rg A = m = rg A^T \Leftrightarrow A^Ty=0\) будем иметь одно решение, по ранее сказанной теореме.
...
\begin{enumerate}
    ...
    \item все элементарные преобразования эквиваленты элементарным преобразованиям со строками матрицы \(A | b\) и перестановкой в ней столбцов (пункт 5).
\end{enumerate}
...
Элементарными преобразованиями системы исходная система может быть замена на эквивалентную систему, матрица которой будет иметь трапециевидную (в частном случае, треугольную) форму.
...
\begin{enumerate}
    \item Прямой ход $(A|b) \sim (\tilde{A}|\tilde{b})$
          ...
          Нужно также учесть, что если в матрице $(\tilde{A}|\tilde{b})$ будет строка вида $(0 \dots 0 | \underbracket{\tilde{b}_j}_{\neq 0})$, то система несовместна.

    \item Обратный ход

          \begin{enumerate}
              \item Вид матрицы треугольный (rg A = n --- числу неизвестных)
                    ...
                    \begin{pmatrix}
                        ...
                        b_n'
                    \end{pmatrix}
                    \) --- решение СЛАУ, причём единственное в силу теоремы о структуре общего решения СЛНУ.


              \item Вид матрицы трапецевидный (rg A < n)

                    Возьму из матрицы треугольник, а остальные переменные возьмём за параметры. Их перенесём в правую часть относительно ``='', затем повторим алгоритм пункта (а). В результате, получим общее решение, зависящее от $n-k$ параметров.
          \end{enumerate}
\end{enumerate}

\subsection{Нахождение обратной матрицы методом Гаусса.}

\(A_{n \times n}\). Найти \(A^{-1}_{n\times n}\), такую, что \(A \cdot A^{-1} = E = A^{-1}\cdot A\)
...
Пусть все системы совместны. Тогда, в процессе нахождения неизвестных столбцов мы делаем с левой частью матрицы одни и те же преобразования. Давайте решать \(n\) систем одновременно:
...
Причем \(A^{-1}\) может быть найдена методом Гаусса.
\begin{enumerate}
    \item[] \prooff{}
          $\exists A^{-1}$, если $\exists$ решения $AX_i = E_i$ $(\forall i = 1 \dots n)$, это значит, что $\rg(A|E_i) = \rg A$ $(\forall i = 1 \dots n)$, откуда каждый $E_i \in span(A_1, \dots, A_n) \subset K^n \Rightarrow rg A = n.$
\end{enumerate}

\textbf{Следствие}. Дано \(A_{n \times n}, Ax=b\). \(A\) обратима \(\Leftrightarrow \exists ^^21\) решение СЛНУ. Причем, \(x = A^{-1} b\)

\(\exists A^{-1} \Leftrightarrow A\) обратима \(\Leftrightarrow\) \(rg A = n \Leftrightarrow\) \(\Leftrightarrow \exists ^^21\) решение СЛНУ \(Ax = b\).
...
\(A_{n\times n}\), A - обратима, \(B \Rightarrow 
\begin{aligned}
    &\rg(AB) = \rg B, \text{если определено A$\cdot$B} \\
    &\rg(BA) = \rg B, \text{если определено B$\cdot$A}
\end{aligned}\)
...
\begin{enumerate}
    ...
    \item
          $\rg(BA) = \rg(BA)^T = \rg(A^T B^T)= \rg(B^T) = \rg B$ (т.к. $\rg(A^T) = \rg(A) = n$)
\end{enumerate}


\subsection{Геометрическая интерпретация СЛАУ}
% TODO: я ничего не понял тут: ни то, что было написано до Екатерины Аркадьевны, ни после. Возможно, нужно будет полностью переписать. Секцию про Геометрическую интерпретацию СЛАУ я скипнул. -- Сергей
\(V, \dim V = n\)
...
\(V, e_1, e_2, \ldots , e_n\) --- ``старый'' базис --- \(e = (e_1, \dots, e_n)\).

\(e_1',e_2',\ldots e_n'\) --- новый базис --- \(e' = (e_1', \dots, e_n')\).

\(x\in V \leftrightarrow \mathbf{x} = \begin{pmatrix}
    \mathbf{x}_1    \\
    \mathbf{x}_2    \\
    \vdots \\
    \mathbf{x}_n
...
\(x = \sum\limits_{i=1}^n \mathbf{x}_i e_i = \sum\limits_{i=1}^n \mathbf{x}_i' e_i'\).
...
$e_j' = \displaystyle \sum_{k=1}^{n} t_{kj}e_k$

\(T = T_{e \rightarrow e'} = (T_1, T_2, \ldots, T_n)\) --- матрица перехода от базиса $e$ к $e'$
...
\begin{enumerate}
    \item \(\rg T = n\) (T обратима) (очевидно, т.к. координатный изоморфизм)


    \item \(T^{-1}\) - матрица перехода из \(e'\) в \(e\).

          Пусть B - матрица перехода от e' к e.

          \((e_1,\ldots,e_n) = (e_1',\ldots,e_n')B = ((e_1,\ldots,e_n)T)B = (e_1,\ldots,e_n) (BT)\), откуда \(BT = E\),  откуда  \(B = T^{-1}\)


    \item связь координат вектора в разных базисах:

          %обозначать координаты X - координатная матрица
          \(x \leftrightarrow \mathbf{x}\) в старом базисе

          \(x \leftrightarrow \mathbf{x}'\) в новом базисе

          \(x = \sum\limits_{i=1}^n \mathbf{x}_i e_i  = \sum \limits_{j=1}^n \mathbf{x}_j'e_j' = \sum\limits_{j=1}^n \mathbf{x}_j' \sum\limits_{i=1}^n t_{ij} e_i = \sum\limits_{i=1}^n (\sum\limits_{j=1}^n t_{ij}x_j')e_i\)

          т.к координаты определяются единственным образом

          \(\forall i = 1\ldots n: \mathbf{x}_i =\sum\limits_{j=1}^n t_{ij} \mathbf{x}_j' = (TX')_i \Leftrightarrow \mathbf{X}=\text{T}\mathbf{X}'\), а

          \(\mathbf{X}'=\text{T}^{-1}\mathbf{X}\)
\end{enumerate}
